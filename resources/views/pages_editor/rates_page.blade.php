        ...
        <div class="overflow-auto row justify-content-center">
            <div class="col-md-12">
                <div class=" secondaryTextColor">
                    <p class="display-3 text-center">{{ __('Rates Page Editor') }}
                    </p>
                    <hr>
                    <div class="card-body">
                        <div class="container mt-2 mb-5">
                            ...
                            <div class="border ">
                                ...
                                <div>
                                    @php
                                        $widgetsData = App\Models\Widget::where('page_id', '3')->get();
                                    @endphp
                        ...
                        <div class="container mt-2 mb-5">
                            ...
                            @php
                                ...
                                $agentRatesPageData = App\Models\AgentPageStaging::where('agent_id', $agentData[0]->id)
                                    ->where('page_id', 3)
                                    ->first();
                            @endphp
                            <textarea style="width: 100%;" class="col-md-12 mt-2" id="ratesPageTemplate">
                                {{ $agentRatesPageData->data }}
                            </textarea>
                            <div class="mt-4 text-center">
                                <a class="btn mt-1 homeButtons" style="background: var(--primary-color)"
                                    onclick="saveRatesPageData({{ $agentRatesPageData->page_id }})">Save Rates Page
                                    Content</a>
                                <a class="btn mt-1 homeButtons" style="background: var(--primary-color)"
                                    onclick="submitForApprovalOfRatesPage({{ $agentRatesPageData->page_id }})">Submit For
                                    Approval</a>
                                <a class="btn mt-1 homeButtons btn-primary"
                                    onclick="setRatesDefaultPage({{ $agentRatesPageData->page_id }})">Default Rates
                                    Page</a>
                                <a class="mt-1 btn btn-dark" target="blank"
                                    href="/admin_dashboard/agent/{{ $agentRatesPageData->page_id }}/view/{{ $agentData[0]->email }}">View
                                    Page</a>
                            </div>
                            <div class="mt-4">
                                <div class="my-1">
                                    @if ($agentRatesPageData->is_approved == 1)
                                        <span class="bg-success lead p-2"
                                            style="color: white;border-radius:7px;">Approved</span>
                                    @elseif ($agentRatesPageData->is_approved == 0 && $agentRatesPageData->is_submitted_for_approval == 0)
                                        ...
                                        <textarea class="mt-0" name="reasonForDisapprovalRatesPage" id="reasonForDisapprovalRatesPage" readonly rows="6"
                                            style="width: 100%; border-radius: 7px;">{{ $agentRatesPageData->reason_for_disapproval }}
                                        </textarea>
                                    @elseif ($agentRatesPageData->is_approved == 0 && $agentRatesPageData->is_submitted_for_approval == 1)
...
@section('scripts')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            ratesPageEditor = $('#ratesPageTemplate').summernote({
                height: 500,
            });
        ...
        function saveRatesPageData(page_id) {
            axios.post("{{ route('saveEditorSubPageDataByAgent') }}", {
                    newData: ratesPageEditor[0].value,
                ...
                .then(function(response) {
                    ...
                    if (response.data == "Saved") {
                        Swal.fire(
                            'Saved!',
                            "Content For Rates Page Saved Successfully",
                            'success'
                        )
                    }
                })
        ...
        function submitForApprovalOfRatesPage(page_id) {
        ...
        function setRatesDefaultPage(pageId) {
            axios.post("{{ route('setAsDefaultPageInPagesEditorViewByAgent') }}", {
                ...
                .then(function(response) {
                    console.log(response);
                    $("#ratesPageTemplate").summernote('code', response.data);
                    Swal.fire(
                        'Successful!',
