                ...
                <div class=" secondaryTextColor">
                    ...
                    <div class="card-body">
                        <div class="container mt-2 mb-5">
                            <p class="text-center display-5">Rates Page Info <i class="fa-solid fa-circle-info"></i> </p>
                            <div class="border ">
                        ...
                        <div class="container mt-2 mb-5">
                            <p class="text-center display-5">Rates Page Content</p>
                            @php
                            ...
                            <textarea style="width: 100%;" class="col-md-12 mt-2" id="ratesPageTemplate">
                                {{ $agentRatePageData->data }}
                            </textarea>
                            <div class="mt-4 text-center">
                                <a class="btn mt-1 homeButtons" style="background: var(--primary-color)"
                                    onclick="saveRatesPageData({{ $agentRatePageData->page_id }})">Save Rates Page
                                    Content</a>
                                <a class="btn mt-1 homeButtons" style="background: var(--primary-color)"
                                    onclick="submitForApprovalOfRatesPage({{ $agentRatePageData->page_id }})">Submit For
                                    Approval</a>
                                <a class="btn mt-1 homeButtons btn-primary"
                                    onclick="setRatesDefaultPage({{ $agentRatePageData->page_id }})">Default Rates
                                    Page</a>
                                <a class="mt-1 btn btn-dark" target="blank"
                                    href="/admin_dashboard/agent/{{ $agentRatePageData->page_id }}/view/{{ $agentData[0]->email }}">View
                                    Page</a>
                            </div>
                            <div class="mt-4">
                                <div class="my-1">
                                    @if ($agentRatePageData->is_approved == 1)
                                        <span class="bg-success lead p-2"
                                            style="color: white;border-radius:7px;">Approved</span>
                                    @elseif ($agentRatePageData->is_approved == 0 && $agentRatePageData->is_submitted_for_approval == 0)
                                        ...
                                        <textarea class="mt-0" name="reasonForDisapprovalHomePage" id="reasonForDisapprovalHomePage" readonly rows="6"
                                            style="width: 100%; border-radius: 7px;">{{ $agentRatePageData->reason_for_disapproval }}
                                        </textarea>
                                    @elseif ($agentRatePageData->is_approved == 0 && $agentRatePageData->is_submitted_for_approval == 1)
            ...
            axios.post("{{ route('saveEditorSubPageDataByAgent') }}", {
                ...
                .then(function(response) {
                    ...
                    if (response.data == "Saved") {
                        Swal.fire(
                            'Saved!',
                            "Content Of Rates Page Saved Successfully",
                            'success'
                        )
                    }
                })
